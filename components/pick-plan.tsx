...
export function PickPlan({ candidates }: { candidates: PlanCandidate[] }) {
  ...
  return (
    <div>
      <h1 className="font-display text-[2.15rem] font-semibold leading-none tracking-tight">Pick a plan</h1>
      <p className="mt-3 max-w-xl text-muted">
        Three ways to spend the same bag of groceries. You can switch later; edits on the old week are dropped.
      </p>
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        {candidates.map((c) => (
          <article key={c.plan_id} className="flex flex-col rounded-[24px] bg-card p-5 shadow-[var(--shadow)]">
            ...
            <button
              ...
              className="mt-auto w-full rounded-full bg-teal py-3 font-semibold text-white disabled:opacity-60"
            >
              Use this week
            </button>
          </article>
